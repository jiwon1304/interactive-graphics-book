...
- **비유(analogy/simile)는 최소.** "문틈으로 들여다보기", "방에 들어가기 전에" 같은 *직유/은유로
  설명을 대체하지 말 것.* 메커니즘은 **직설적으로** 설명한다(무엇이·왜·어떻게). 단, **구체적 실례**
  (멀리 있는 철망이 지글거린다 / PS1 텍스처가 휜다)는 비유가 아니라 *현상 자체*이므로 OK.
  "직관 손잡이" 박스를 비유 나열로 채우지 말고, 꼭 필요할 때 *짧은 직관 1문장*만.
