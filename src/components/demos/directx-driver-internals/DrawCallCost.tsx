...
// 드로우 콜 CPU 비용(인터랙티브). draw 수 슬라이더 → DX11 vs DX12 프레임 CPU 시간을
// 범주별(검증/상태·hazard/디스크립터/제출)로 누적 막대. 16.6ms(60fps) 예산선.
//
// 대표 차수의 도식용 모델(특정 드라이버 측정 아님): 핵심은 절대값이 아니라 per-draw 합과 기울기.

const CATS = [
  { key: '검증', c: COLORS.validate, dx11: 180, dx12: 30 },
  { key: '상태·hazard', c: COLORS.state, dx11: 340, dx12: 25 },
  { key: '디스크립터', c: COLORS.descriptor, dx11: 240, dx12: 15 },
  { key: '제출(amortized)', c: COLORS.submit, dx11: 140, dx12: 30 },
]; // ns/draw
...
export default function DrawCallCost() {
  ...
  const draw = (d: DrawCtx) => {
    ...
    const barMaxW = w - barX - pad - 70;
    const dx11ms = totals('dx11');
    const dx12ms = totals('dx12');
    const scaleMax = Math.max(dx11ms, BUDGET_MS) * 1.08;
    const msToPx = barMaxW / scaleMax;

    const barH = 38;
    const drawBar = (y: number, name: string, nameC: string, which: 'dx11' | 'dx12', totalMs: number) => {
      label(ctx, pad + leftW / 2 - 4, y + barH / 2, name, nameC, 13, 'bold');
      let x = barX;
      CATS.forEach((c) => {
    ...
    drawBar(24, 'DX11', COLORS.dx11, 'dx11', dx11ms);
    drawBar(24 + barH + 26, 'DX12', COLORS.dx12, 'dx12', dx12ms);
    ...
    ctx.beginPath();
    ctx.moveTo(bx, 14);
    ctx.lineTo(bx, 24 + barH * 2 + 26 + 6);
    ctx.stroke();
    ctx.setLineDash([]);
    label(ctx, bx, 8, '16.6ms (60fps)', COLORS.submit, 9, 'bold');

    // 범례
    const ly = 24 + barH * 2 + 26 + 24;
    let lx = barX;
    ...
    CATS.forEach((c) => {
      ...
      ctx.textBaseline = 'alphabetic';
      lx += 16 + ctx.measureText(c.key).width + 18;
    });
  };
  ...
  return (
    <figure className="demo">
      ...
      <figcaption>
        ...
        차지합니다 — 드라이버가 매 draw마다 바인딩을 검사·변환하기 때문입니다. DX12는 이 일들을 PSO 생성
        시점(프레임 밖)으로 옮기고 descriptor heap/root signature로 패치를 없애, draw당 비용이 거의
        제출만 남습니다. 그래서 draw가 많아질수록 DX11은 <span style={{ color: COLORS.submit }}>16.6ms
        예산</span>을 먼저 넘고(현재 DX11 {dx11ms.toFixed(1)}ms vs DX12 {dx12ms.toFixed(1)}ms), DX12는
        같은 장면을 한참 더 끌고 갑니다. DX12가 “빠르다”는 건 GPU가 아니라 <strong>이 CPU 제출
        오버헤드</strong>가 얇아졌다는 뜻입니다.
      </figcaption>
    </figure>
  );
}
