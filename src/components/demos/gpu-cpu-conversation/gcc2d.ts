// gpu-cpu-conversation 챕터 공용 2D 유틸.
// CPU↔GPU 비동기 타임라인 · command buffer ring · user/kernel 드라이버 분담 ·
// Draw() → command buffer 바이트 도식이 공유한다.
//
// dxd2d.ts를 복사·적응했고, 좁은 폭(360~440px)에서 긴 라벨을 박스 안에 맞추기 위해
// wrapText(ctx,text,x,y,maxW,lineH)를 추가했다. 전부 벡터 도식이라 putImageData를 쓰지 않는다
// (AUTHORING §5.1 무관). setupCanvas로 dpr 변환을 건 ctx에 곧장 그린다. 캔버스 안 글자는 짧은
// 노드명/수치만, 설명은 MDX figcaption.
...
/** 의미색(테마 무관). CPU/GPU 레인 · 스택 레이어 · 명령 종류를 일관된 색으로. */
export const COLORS = {
  // 두 프로세서
  cpu: '#3b82f6', // CPU — 파랑
  gpu: '#f59e0b', // GPU — 주황
  present: '#22c55e', // present / 완료 — 초록
  // 스택 레이어 (user/kernel)
  app: '#64748b', // 애플리케이션 — 회색
  runtime: '#3b82f6', // runtime — 파랑
  umd: '#14b8a6', // user-mode driver — 청록
  kernel: '#a855f7', // kernel / OS scheduler — 보라
  // 명령 / 동기화
  cmd: '#0ea5e9', // command bytes — 하늘
  fence: '#ef4444', // fence / 대기 — 빨강
  idle: '#94a3b8', // idle / stall — 흐린 회색
} as const;
...
/**
 * 폭 maxW에 맞춰 단어 단위로 줄바꿈하며 (x,y)에서 위→아래로 그린다.
 * 좁은 모바일 폭에서 긴 라벨을 박스 안에 맞추기 위함. 그린 줄 수를 반환.
 * 폰트/색/정렬은 호출 전에 ctx에 설정해 둘 것(이 함수는 fillText만 한다).
 */
