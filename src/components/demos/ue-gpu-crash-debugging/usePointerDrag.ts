 ...
 * 점이 안 따라오는" 증상이 난다. 캔버스에 네이티브 리스너를
 * { passive: false }로 직접 붙이고 preventDefault를 호출해 막는다.
 *
 * (raymarching-sdf/usePointerDrag.ts와 동일 패턴 — 챕터 폴더 자기완결성을
 *  위해 사본을 둠. 동작·시그니처는 같다.)
 */
  ...
  useEffect(() => {
    ...
    const onDown = (e: PointerEvent) => {
      ...
      try {
        ...
        /* iOS 일부 버전에서 throw 가능 — window 리스너로 추적되므로 무시 */
      }
    };
